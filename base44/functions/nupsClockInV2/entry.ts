...
//   constant-time comparison. NO plaintext fallback. Lookup via peppered HMAC index (pin_lookup_v2) —
...
// - Kiosk sessions: NKS2.<payload>.<sig> HMAC-SHA256 signed credential carrying uid, role, venue,
...
Deno.serve(async (req) => {
  try {
    ...
    const resolveUserByPin = async (pin) => {
      ...
      const candidates = (await E.NUPSUser.filter({ pin_lookup_v2: lookup })) || [];
    ...
    if (action === 'adminSetPin') {
      ...
      await E.NUPSUser.update(nups_user_id, { pin_hash, pin_salt, pin_lookup_v2: lookup, pin_must_change: false, pin_changed_at: now() });
    ...
    if (action === 'changeTemporaryPin') {
      ...
      await E.NUPSUser.update(staff.id, { pin_hash, pin_salt, pin_lookup_v2: lookup, pin_must_change: false, pin_changed_at: now(), pin_failed_attempts: 0 });
