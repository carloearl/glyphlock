  ...
  const handleUpload = async () => {
    ...
    try {
      if (useDrive) {
        ...
        const result = await base44.functions.invoke('uploadToDrive', formData);
        const data = result?.data;
        
        const driveUrl = data?.file_url || data?.download_url;
        if (driveUrl) {
          setFileUrl(driveUrl);
          setPreviewUrl(data.embed_url || data.preview_url || '');
          localStorage.setItem('gl_last_uploaded_url', driveUrl);
